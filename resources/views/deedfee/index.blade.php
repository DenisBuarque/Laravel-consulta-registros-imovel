        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <a href="{{ route('deedfee.index') }}">Lista de Taxas de escritura</a> cadastrados
                    @can('create-data')
                        <a href="{{ route('deedfee.create') }}" class="pull-right">Adicionar Registro</a>
                    @endcan
                </div>
                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($deedfees as $key => $value)
                                    <tr>
                                        <td class="text-center">{{ number_format($value->vl_referencia,2,',','.') }}</td>
                                        <td class="text-center">{{ number_format($value->emolumento,2,',','.') }}</td>
                                        <td class="text-center">{{ number_format($value->selo,2,',','.') }}</td>
                                        <td class="text-center">{{ number_format($value->vl_total,2,',','.') }}</td>
                                    
                                        <td class="text-center">
                                            @can('update-data')
                                                <a href="{{ route('deedfee.edit', $value->id) }}" class="btn btn-xs btn-success">
                    ...
                    @if($deedfees) 
                        <div class="">{{ $deedfees->links() }}</div>
                    @endif

                    @foreach($deedfees as $value)
                        <!-- Modal -->
                            ...
                            <div class="modal-dialog modal-sm" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form method="POST" action="{{ route('deedfee.delete', $value->id) }}">
